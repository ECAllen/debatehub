...
func (v ThreadsResource) List(c buffalo.Context) error {
	...
	err := q.All(threads)
	// to:
	// err := q.Order("created_at desc").All(threads)
	if err != nil {
		return errors.WithStack(err)
	}
	// Make Threads available inside the html template
	c.Set("threads", threads)
	// Add the paginator to the context so it can be used in the template.
	c.Set("pagination", q.Paginator)
	return c.Render(200, r.HTML("threads/index.html"))
}
...
func (v ThreadsResource) Show(c buffalo.Context) error {
	...
	err := tx.Find(thread, c.Param("thread_id"))
	if err != nil {
		return errors.WithStack(err)
	}
	// Make thread available inside the html template
	c.Set("thread", thread)
	return c.Render(200, r.HTML("threads/show.html"))
}
...
func (v ThreadsResource) New(c buffalo.Context) error {
	// Make thread available inside the html template
	c.Set("thread", &models.Thread{})
	return c.Render(200, r.HTML("threads/new.html"))
}
...
func (v ThreadsResource) Edit(c buffalo.Context) error {
	...
	// Make thread available inside the html template
	c.Set("thread", thread)
	return c.Render(200, r.HTML("threads/edit.html"))
}
...
func (v ThreadsResource) Update(c buffalo.Context) error {
	...
	if verrs.HasAny() {
		...
		// correct the input.
		return c.Render(422, r.HTML("threads/edit.html"))
	}
	// If there are no errors set a success message
